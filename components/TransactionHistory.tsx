...
const TransactionHistory: React.FC<TransactionHistoryProps> = ({ transactions, onEdit, onDelete, currentUser }) => {
  ...
  const filteredTransactions = transactions
    .filter(t => {
      ...
      if (searchTerm && !t.product_name?.toLowerCase().includes(searchTerm.toLowerCase())) return false;
  ...
  const handleExport = () => {
    const headers = ["Tarih", "İşlem Tipi", "Ürün", "Miktar", "Önceki Stok", "Sonraki Stok", "Açıklama", "Kullanıcı"];
          ...
          {showFilters && (
              <div className="pt-4 border-t border-slate-100 dark:border-slate-700 animate-fade-in">
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
                      <div>
                          <label className="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">Açıklama İçerir</label>
                          <input 
                              type="text" 
                              value={descSearch}
        ...
        {filteredTransactions.map(t => (
          <div key={t.id} className="bg-white dark:bg-slate-800 p-4 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700 flex justify-between items-center relative overflow-hidden group isolate transition-colors">
            ...
            <div className="pl-3 flex-1">
              ...
              <h4 className="font-bold text-slate-800 dark:text-white">{t.product_name}</h4>
              <p className="text-sm text-slate-500 dark:text-slate-400 line-clamp-1">{t.description}</p>
              <p className="text-xs text-slate-400 dark:text-slate-500 mt-1">İşlem Yapan: {t.created_by}</p>
            </div>
