...
const BarcodeScanner: React.FC<BarcodeScannerProps> = ({ onScanSuccess, onScanFailure, onClose }) => {
  ...
  const toggleTorch = () => {
      if (scannerRef.current && hasTorch) {
          ...
          const constraints = { advanced: [{ torch: newStatus }] } as any;
  ...
  const handleZoomChange = (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (scannerRef.current) {
          ...
          scannerRef.current.applyVideoConstraints(constraints).catch(err => console.error(err));
      }
  };
